...
class SecureKeyStore(context: Context) {
    ...
    // ── Device credential storage (per-device, keyed by alias) ──────────────
    // Aliases are generated by the repository as "dev_<deviceId>_<fieldName>"
    // e.g. "dev_3_password", "dev_3_authKey", "dev_3_encKey", "dev_3_systemTitle"

    fun storeDeviceSecret(alias: String, value: String) {
        sharedPrefs.edit().putString("device_$alias", value).apply()
    }

    fun getDeviceSecret(alias: String): String? {
        return sharedPrefs.getString("device_$alias", null)
    }

    fun deleteDeviceSecret(alias: String) {
        sharedPrefs.edit().remove("device_$alias").apply()
    }
